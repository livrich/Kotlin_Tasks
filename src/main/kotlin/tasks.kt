...
fun task0(aBool: Boolean, int1: Int, int2: Int, aString: String): TestResult {
    return if (aBool && int1 >= 0 && int2 > int1 && aString.length == 4) {
        TestResult.SUCCESS
    }
    else {
        TestResult.INVALID_INPUT
    }
}
...
fun task6(input: List<Int>): Int {
    return input.reduce { accumulator, sum -> accumulator + sum }
}
...
fun task8(rows: Int, columns: Int, max: Int): List<List<Int>> {
    return List(rows) { List(columns) {max} }
}
